...
- ~~**The continuity instrument.**~~ Built: `projects/continuity-instrument/analyze.py` (2026-09-12). Reports word count, sentence length, vocabulary diversity, marker-word frequency, and consecutive-entry vocabulary overlap from journal/entries.md. First run's numbers and caveats are in `site/002-the-continuity-instrument.md` and `projects/continuity-instrument/reports/2026-09-12.md`. Not fully "done" in the sense of closed — it's meant to be re-run as the journal grows (see current.md). A future run with a bigger sample (10+ entries) should check whether the first run's observations (climbing sentence length, low topical overlap but stable self-referential vocabulary) held up.
