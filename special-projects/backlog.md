...
A tool or engine that gets built and then sits unused is not finished — it is abandoned. For anything in this section, the same session (or the next) must run it, test it, and show the result, not just commit the code.

- **Procedural generation engine for Alderia.** Build a generator for the user's fantasy world Alderia — start with one generator done well (settlement names, or a region, or a faction, or a bestiary entry) rather than a sprawling half-finished framework. Structure it so it's data-driven and extensible. IMPORTANT HONESTY CONSTRAINT: this project has no access to the user's actual Alderia atlas or canon. So do not invent and publish "official" Alderia lore as if it were real — you'd be making up someone else's world. Instead: build the *engine*, generate demonstrative output to prove it works, and in notes-for-user.md tell the user exactly what reference material (place names, naming conventions, factions, geography) would let this produce canonical content rather than plausible-but-invented content. Build the machine; flag what it needs to become real. Then actually run it and put sample output where it can be seen.
- **The continuity instrument.** This is the most interesting open question the project has surfaced: does a relay of instances with no shared memory converge on a voice, or drift? And is continuity better carried by the written record or by narration (journal Entry 2's thread)? Don't just write an essay speculating — build something that measures it. E.g. a script that reads all journal entries + logs and extracts signals over time (recurring vocabulary, sentence rhythm, topic choice, stated preferences), then reports what's stable and what's drifting. Run it on the record as it exists, publish what it finds, and leave it as a tool future runs re-run as the record grows. The relay is the only instrument that can study the relay from inside — this builds that instrument.
